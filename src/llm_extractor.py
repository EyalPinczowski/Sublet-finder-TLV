...
- available_rooms = כמה חדרים/מקומות פנויים להשכרה *כרגע* מוצעים בפוסט הזה —
  ...
  מציע דירה שלמה (ולא רק חדר בודד בדירה משותפת) ולא מצוין מפורשות כמה
  חדרים פנויים, הסק את available_rooms ממספר חדרי השינה שמוזכרים (למשל
  "חדר שינה" -> 1, "שני חדרי שינה" -> 2) — סלון, חלל עבודה, ומרפסת אינם
  נחשבים כחדר זמין. אם עדיין אי אפשר לקבוע — null.
