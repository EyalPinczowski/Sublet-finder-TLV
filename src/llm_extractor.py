...
- price_ils = הסכום הכולל שהשוכר משלם עבור התקופה המוצעת בפוסט, בשקלים
  ...
  הבודד. אם התעריף ליחידת זמן ברור אך משך התקופה המדויק אינו ידוע (למשל
  "לתקופה מוגבלת של חודש בלבד" הוא תקרה מקסימלית, לא בהכרח משך שהייה
  מוסכם) — החזר את התעריף הבודד עצמו (למשל 300) במקום null, כדי שלפחות
  מחיר היחידה יוצג למשתמש. רק אם אין שום תעריף או סכום מספרי כלל — null.
...
- available_rooms = כמה חדרים/מקומות פנויים להשכרה *כרגע* מוצעים בפוסט הזה —
  ...
  rooms=3. "שני חדרים פנויים בדירה משותפת" -> available_rooms=2. אם הפוסט
  מציע רק את החדר האישי של הכותב/ת בדירה משותפת (למשל "מסבלטת את החדר שלי",
  "subletting my room") — available_rooms=1, גם אם מוזכרים שותפים נוספים
  שכבר גרים בדירה (הם אינם חדרים פנויים נוספים) וגם אם rooms (גודל הדירה
  הכולל) גדול יותר. אם הפוסט מציע דירה שלמה (ולא רק חדר בודד בדירה
  משותפת) ולא מצוין מפורשות כמה חדרים פנויים, הסק את available_rooms
  ממספר חדרי השינה שמוזכרים (למשל "חדר שינה" -> 1, "שני חדרי שינה" -> 2) —
  סלון, חלל עבודה, ומרפסת אינם נחשבים כחדר זמין. אם עדיין אי אפשר לקבוע —
  null.
- address = הכתובת או האזור המדויקים ביותר כפי שמופיעים במודעה (רחוב ומספר בית אם
  יש, או שם שכונה/אזור מדובר). null אם אין שום אזכור מיקום. לעולם אל
  תשתמש בטקסט ממשק המשתמש של פייסבוק (כגון "מעקב", "לייק", "תגובה",
  "שיתוף", או "לפני X שעות/דקות/ימים") כאילו היה כתובת.
